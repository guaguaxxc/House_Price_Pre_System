...
================================================
提供了两个主要映射字典：
    - CITY_TO_PROVINCE：将城市名映射到其所属的省份/直辖市/自治区
    - PROVINCE_TO_REGION：将省份名映射到其所属的大区（如华北、华东等）
以及一个标准化省份名称的函数，用于将带后缀的省份名称转换为简写形式。
"""
...
# ================= 省份到区域的映射字典 =================
# 用于将省份名（简化后，如“辽宁”、“北京”）映射到其所处的经济地理大区。
PROVINCE_TO_REGION = {
    # 东北
    "辽宁": "东北", "吉林": "东北", "黑龙江": "东北",
    # 华北
...
def standardize_province_name(province):
    """
    将带后缀的省份名称转换为简化版，以便与 PROVINCE_TO_REGION 的键匹配。
    例如：
        "辽宁省" → "辽宁"
        "广西壮族自治区" → "广西"
        "北京市" → "北京"
        "香港特别行政区" → "香港"
    :param province: str - 原始省份名称（可能包含“省”、“自治区”、“市”等后缀）
    :return: str - 移除后缀后的简化省份名
    """
    # 需要移除的后缀列表
